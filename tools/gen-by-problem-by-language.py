        ...
        for sym in all_symptoms:
            ...
            table_rows.append(
                (
                    short_sym,
                    p["name"],
                    f"../patterns/{p['family_dir']}/{p['file_name']}",
                    p["family_name"],
                )
            )
...
def build_language_doc(patterns: list[dict]) -> str:
    ...
    headers = ["Pattern", "Family"] + [
        TARGET_LANGS[lang_key] for lang_key in langs_sorted
    ]
    ...
    for p in sorted(patterns, key=lambda x: (x["family_dir"], x["name"])):
        ...
        for lang_key in langs_sorted:
            row.append("Yes" if lang_key in p["impl_langs"] else "")
    ...
    for lang_key in langs_sorted:
        ...
        lines.append("")
        lines.append(
            f"These patterns contain runnable code examples written in {lang_name}:"
        )
        lines.append("")
        if impls:
        ...
        lines.append("")
        lines.append(
            f"These patterns have unique implementation variants or change their design structure specifically when built using {lang_name} features (documented in Dimension 8):"
        )
        lines.append("")
        if shapes:
        ...
        lines.append("")
        lines.append(
            f"These patterns have native features in {lang_name} that make the pattern unnecessary, or require an alternative design approach (documented in Dimension 4):"
        )
        lines.append("")
        if unnecessaries:
...
def main() -> int:
    ...
    pattern_files = [
        f for f in pattern_files if f.name.lower() not in {"readme.md", "index.md"}
    ]
